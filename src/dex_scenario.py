...
def main():
    ...
    if len(offers) == 0:
        ...
        for o in offers:
            if Decimal(o["quality"]) <= proposed_quality:
                print(
                    f"Matching Offer found, funded with {o.get('owner_funds')} "
                    f"{we_want['currency']}"
                )
        ...
        if 0 < running_total < want_amt:
            print(
                f"Remaining {want_amt - running_total} {we_want['currency']} "
    ...
    if running_total == 0:
        ...
        print(f"Orderbook2:\n{pprint.pformat(orderbook2_info)}")
        ...
        if len(offers2) == 0:
            ...
            for o in offers2:
                if Decimal(o["quality"]) <= offered_quality:
                    print(
                        f"Existing offer found, funded with {o.get('owner_funds')} "
                        f"{tally_currency}"
                    )
            ...
            print(
                f"Our Offer would be placed below at least {running_total2} "
                f"{tally_currency}"
            )
            if 0 < running_total2 < want_amt:
                print(
                    f"Remaining {want_amt - running_total2} {tally_currency} "
    ...
    for affnode in result["meta"]["AffectedNodes"]:
        if "ModifiedNode" in affnode:
            ...
            if affnode["CreatedNode"]["LedgerEntryType"] == "RippleState":
                ...
                print(
                    f"Created an Offer owned by {offer['Account']} with "
    ...
    # # Check Offers --------------------------------------------------------------
    # print(
    #     f"Getting outstanding Offers from {account.address} "
    ...
    #         print(
    #             f"Total matched: {min(running_total, want_amt)} {we_want['currency']}"
    #         )
    #         if 0 < running_total < want_amt:
    #             print(
    #                 f"Remaining {want_amt - running_total} {we_want['currency']} "
    ...
    #     # Check Offers --------------------------------------------------------------
    #     print(
    #         f"Getting outstanding Offers from {wallet.address} "
    #         f"as of validated ledger..."
    #     )
    #     acct_offers = await client.request(
    #         AccountOffers(
